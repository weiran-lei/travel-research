...
- Register every supplied file by its original filename and record whether it was processed, partially readable, or unreadable. When the host provides a stable usable path or locator, record it without changing the filename ID.
...
For an existing version 0.4–0.7 configuration, preserve its choices and apply these presentation defaults unless the user chooses otherwise:
...
1. Register all sources using their original filenames as stable IDs. Add the optional `locator` only when the host supplies a stable usable path or URI; do not fabricate one or copy the source to create one.
...
4. Resolve Country → Destination/City/Region → Place only when supported. Put facts that apply to an entire country in `country_information`; do not duplicate them across destinations or invent a destination or place. Keep uncertain identity or geography in an unconfirmed section.
...
- Display every source using its full original filename as the label. Make it clickable using its registered `locator` when available; otherwise show the filename as plain text.
...
Every JSON source reference must exactly match an ID in the source registry. A source `locator` is optional and must not replace its filename ID. Do not use empty strings for missing values. Coverage remains mandatory for represented geographic levels in JSON because it supports validation, even when hidden from the reading output.
